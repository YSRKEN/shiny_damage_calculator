...
export const calcNormalDamage = (
	...
	const p = getStatus(state.pIdolStatus, cardType);
	const s = state.sIdolStatus.map(status => getStatus(status, cardType))
		.reduce((previous, current) => previous + current);
	const w = state.auditionWeek;
	const buff1 = 1.0 * (100 + getStatus(state.buffValue, cardType)) / 100;
	...
	if (idolIndex === 0) {
		...
		// sアイドルのアピール値
		const is = getStatus(state.sIdolStatus[idolIndex - 1], cardType);
